...
The `tq validate` command checks all active (non-archived) tickets for referential integrity. It scans every ticket's `deps`, `parent`, and `links` fields to ensure each referenced ticket ID exists. Archived tickets are excluded from validation but are recognized as reference targets so the system can distinguish a truly missing ticket from one that has been archived.

Problems are classified as violations (errors) or warnings. A missing reference is a violation; a reference to an archived ticket is a warning. The command exits non-zero only when at least one violation is found.

## Scenarios

### Requirement: Validate command

The system SHALL provide a `tq validate` command that checks every active ticket for integrity problems and reports all violations found.
...
#### Scenario: Summary singular forms
- Given 1 violation and 1 warning exist
- When the user runs `tq validate`
- Then the last line of stderr is `1 error, 1 warning`

### Requirement: Dependency existence

WHEN a ticket's `deps` list references a ticket ID, the system SHALL verify that the referenced ticket exists among active tickets. IF the referenced dependency does not exist at all, the system SHALL report a violation. IF the referenced dependency exists only as an archived ticket, the system SHALL report a warning.

#### Scenario: Valid dependency
- Given ticket "proj-a001" exists with `deps: [proj-b002]`
- And ticket "proj-b002" exists (active)
- When the user runs `tq validate`
...
- And ticket "proj-p001" exists (active)
- When the user runs `tq validate`
...
WHEN a ticket's `links` list references a ticket ID, the system SHALL verify that the referenced ticket exists among active tickets. IF the referenced link target does not exist at all, the system SHALL report a violation. IF the referenced link target exists only as an archived ticket, the system SHALL report a warning.
...
The system SHALL check only active tickets. Tickets under `.tickets/archive/` SHALL be excluded from validation but SHALL be recognized when determining whether a reference target exists (to distinguish "missing" from "archived").
...
- And no active tickets have violations
- When the user runs `tq validate`
- Then the command exits zero
