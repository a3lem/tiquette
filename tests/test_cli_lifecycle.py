...
class TestCreateBehavior:
    ...
    # spec: ticket-lifecycle requirement=create-ticket scenario=create-with-title
    def test_create_prints_id_and_exits_zero(
        self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        monkeypatch.chdir(tmp_path)
        result = run_tq(
            "create", "My first ticket", env={"TICKETS_DIR": str(tmp_path / ".tickets")}
        )
        assert result.returncode == 0
    ...
    # spec: ticket-lifecycle requirement=create-ticket scenario=create-with-title
    def test_create_writes_file_with_title(
        self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        monkeypatch.chdir(tmp_path)
        tickets_dir = tmp_path / ".tickets"
        result = run_tq(
            "create", "My first ticket", env={"TICKETS_DIR": str(tickets_dir)}
        )
        ticket_id = result.stdout.strip()
    ...
    # spec: ticket-lifecycle requirement=create-ticket scenario=create-without-title-is-rejected
    def test_create_no_title_rejected(
        self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        monkeypatch.chdir(tmp_path)
        tickets_dir = tmp_path / ".tickets"
    ...
    def test_create_with_type(
        ...
        tickets_dir = tmp_path / ".tickets"
        result = run_tq(
            "create", "Bug", "-t", "bug", env={"TICKETS_DIR": str(tickets_dir)}
        )
        ticket_id = result.stdout.strip()
        content = _read_ticket_file(tickets_dir, ticket_id)
    ...
    def test_create_with_priority(
        ...
        tickets_dir = tmp_path / ".tickets"
        result = run_tq(
            "create", "Urgent", "-p", "0", env={"TICKETS_DIR": str(tickets_dir)}
        )
        ticket_id = result.stdout.strip()
        content = _read_ticket_file(tickets_dir, ticket_id)
    ...
    def test_create_with_assignee(
        ...
        tickets_dir = tmp_path / ".tickets"
        result = run_tq(
            "create", "Task", "-A", "John Doe", env={"TICKETS_DIR": str(tickets_dir)}
        )
        ticket_id = result.stdout.strip()
        content = _read_ticket_file(tickets_dir, ticket_id)
    ...
    def test_create_auto_creates_tickets_dir(
        self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        monkeypatch.chdir(tmp_path)
        tickets_dir = tmp_path / ".tickets"
...
class TestStatusTransitionBehavior:
    ...
    def test_close_rejects_parent_with_open_children(self, tmp_path: Path) -> None:
        ...
        write_ticket(
            _make_ticket("test-0002", title="Child", parent="test-0001"), tickets_dir
        )
        result = run_tq("close", "test-0001", env={"TICKETS_DIR": str(tickets_dir)})
        assert result.returncode != 0
    ...
    def test_close_succeeds_when_all_children_closed(self, tmp_path: Path) -> None:
        ...
        write_ticket(_make_ticket("test-0001"), tickets_dir)
        write_ticket(
            _make_ticket("test-0002", status="closed", parent="test-0001"), tickets_dir
        )
        result = run_tq("close", "test-0001", env={"TICKETS_DIR": str(tickets_dir)})
        assert result.returncode == 0

    # spec: ticket-lifecycle requirement=close-command scenario=close-rejects-grandparent-with-non-terminal-grandchild
    def test_close_rejects_grandparent_with_open_grandchild(
        self, tmp_path: Path
    ) -> None:
        from tiquette.store import write_ticket
        ...
        write_ticket(_make_ticket("test-0001"), tickets_dir)
        write_ticket(
            _make_ticket("test-0002", status="canceled", parent="test-0001"),
            tickets_dir,
        )
    ...
    def test_close_notifies_last_open_child(self, tmp_path: Path) -> None:
        ...
        write_ticket(_make_ticket("test-0001", title="Parent"), tickets_dir)
        write_ticket(
            _make_ticket("test-0002", title="Last child", parent="test-0001"),
            tickets_dir,
        )
    ...
    def test_force_cancel_cascades(self, tmp_path: Path) -> None:
        ...
        write_ticket(
            _make_ticket("test-0003", status="in_progress", parent="test-0002"),
            tickets_dir,
        )
        result = run_tq(
            "cancel", "-f", "test-0001", env={"TICKETS_DIR": str(tickets_dir)}
        )
        assert result.returncode == 0
    ...
    def test_force_cancel_skips_closed_descendants(self, tmp_path: Path) -> None:
        ...
        result = run_tq(
            "cancel", "--force", "test-0001", env={"TICKETS_DIR": str(tickets_dir)}
        )
        assert result.returncode == 0
    ...
    def test_force_close_cascades(self, tmp_path: Path) -> None:
        ...
        result = run_tq(
            "close", "-f", "test-0001", env={"TICKETS_DIR": str(tickets_dir)}
        )
        assert result.returncode == 0
        for tid in ("test-0001", "test-0002", "test-0003"):
    ...
    def test_force_close_skips_closed_descendants(self, tmp_path: Path) -> None:
        ...
        result = run_tq(
            "close", "--force", "test-0001", env={"TICKETS_DIR": str(tickets_dir)}
        )
        assert result.returncode == 0
...
class TestTransitionOutput:
    ...
    def test_force_close_prints_all_ids(self, tmp_path: Path) -> None:
        ...
        result = run_tq(
            "close", "-f", "par-0001", env={"TICKETS_DIR": str(tickets_dir)}
        )
        assert result.returncode == 0
    ...
    def test_force_cancel_prints_all_ids(self, tmp_path: Path) -> None:
        ...
        write_ticket(_make_ticket("par-0002", parent="par-0001"), tickets_dir)
        result = run_tq(
            "cancel", "-f", "par-0001", env={"TICKETS_DIR": str(tickets_dir)}
        )
        assert result.returncode == 0
        assert "par-0001" in result.stdout
        assert "par-0002" in result.stdout
...
class TestCreateWithLink:
    ...
    def test_create_with_link_symmetric(
        self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        monkeypatch.chdir(tmp_path)
        tickets_dir = tmp_path / ".tickets"
    ...
    # spec: ticket-lifecycle requirement=create-ticket scenario=create-with-links-is-symmetric
    def test_create_with_link_arg_accepted(
        self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        monkeypatch.chdir(tmp_path)
        tickets_dir = tmp_path / ".tickets"
...
class TestMultiIdTransitions:
    """Lifecycle commands accept multiple IDs with atomic, fail-fast semantics."""

    # spec: ticket-lifecycle requirement=start-command scenario=start-multiple-tickets
    def test_start_multiple(self, tmp_path: Path) -> None:
        from tiquette.store import read_ticket, write_ticket

        tickets_dir = tmp_path / ".tickets"
        tickets_dir.mkdir()
        write_ticket(_make_ticket("test-0001"), tickets_dir)
        write_ticket(_make_ticket("test-0002"), tickets_dir)
        result = run_tq(
            "start", "test-0001", "test-0002", env={"TICKETS_DIR": str(tickets_dir)}
        )
        assert result.returncode == 0
        assert read_ticket("test-0001", tickets_dir).status == "in_progress"
        assert read_ticket("test-0002", tickets_dir).status == "in_progress"

    # spec: ticket-lifecycle requirement=close-command scenario=close-multiple-tickets
    def test_close_multiple(self, tmp_path: Path) -> None:
        ...
        write_ticket(_make_ticket("test-0002"), tickets_dir)
        result = run_tq(
            "close", "test-0001", "test-0002", env={"TICKETS_DIR": str(tickets_dir)}
        )
        assert result.returncode == 0
        assert read_ticket("test-0001", tickets_dir).status == "closed"
        assert read_ticket("test-0002", tickets_dir).status == "closed"

    # spec: ticket-lifecycle requirement=cancel-command scenario=cancel-multiple-tickets
    def test_cancel_multiple(self, tmp_path: Path) -> None:
        from tiquette.store import read_ticket, write_ticket
        ...
        write_ticket(_make_ticket("test-0002"), tickets_dir)
        result = run_tq(
            "cancel", "test-0001", "test-0002", env={"TICKETS_DIR": str(tickets_dir)}
        )
        assert result.returncode == 0
        assert read_ticket("test-0001", tickets_dir).status == "canceled"
        assert read_ticket("test-0002", tickets_dir).status == "canceled"

    # spec: ticket-lifecycle requirement=reopen-command scenario=reopen-multiple-tickets
    def test_reopen_multiple(self, tmp_path: Path) -> None:
        from tiquette.store import read_ticket, write_ticket

        tickets_dir = tmp_path / ".tickets"
        tickets_dir.mkdir()
        write_ticket(_make_ticket("test-0001", status="closed"), tickets_dir)
        write_ticket(_make_ticket("test-0002", status="canceled"), tickets_dir)
        result = run_tq(
            "reopen", "test-0001", "test-0002", env={"TICKETS_DIR": str(tickets_dir)}
        )
        assert result.returncode == 0
        assert read_ticket("test-0001", tickets_dir).status == "open"
        assert read_ticket("test-0002", tickets_dir).status == "open"

    # spec: ticket-lifecycle requirement=close-command scenario=close-with-one-unknown-id-writes-nothing
    # spec: ticket-lifecycle requirement=invalid-operations scenario=non-existent-ticket-among-valid-ones
    def test_close_unknown_among_valid_writes_nothing(self, tmp_path: Path) -> None:
        from tiquette.store import read_ticket, write_ticket
        ...
        write_ticket(_make_ticket("test-0001"), tickets_dir)
        result = run_tq(
            "close", "test-0001", "nonexistent", env={"TICKETS_DIR": str(tickets_dir)}
        )
        assert result.returncode != 0
        assert "not found" in result.stderr
        assert read_ticket("test-0001", tickets_dir).status == "open"

    # spec: ticket-lifecycle requirement=start-command scenario=start-with-one-unknown-id-writes-nothing
    def test_start_unknown_among_valid_writes_nothing(self, tmp_path: Path) -> None:
        from tiquette.store import read_ticket, write_ticket
        ...
        write_ticket(_make_ticket("test-0001"), tickets_dir)
        result = run_tq(
            "start", "test-0001", "nonexistent", env={"TICKETS_DIR": str(tickets_dir)}
        )
        assert result.returncode != 0
        assert read_ticket("test-0001", tickets_dir).status == "open"

    # spec: ticket-lifecycle requirement=close-command scenario=close-batch-aborts-when-one-target-has-non-terminal-children
    def test_close_batch_aborts_when_target_blocked(self, tmp_path: Path) -> None:
        from tiquette.store import read_ticket, write_ticket

        tickets_dir = tmp_path / ".tickets"
        tickets_dir.mkdir()
        write_ticket(_make_ticket("test-0001"), tickets_dir)
        write_ticket(_make_ticket("par-0001", title="Parent"), tickets_dir)
        write_ticket(
            _make_ticket("par-0002", title="Child", parent="par-0001"), tickets_dir
        )
        result = run_tq(
            "close", "test-0001", "par-0001", env={"TICKETS_DIR": str(tickets_dir)}
        )
        assert result.returncode != 0
        assert "par-0001" in result.stderr
        assert read_ticket("test-0001", tickets_dir).status == "open"
        assert read_ticket("par-0001", tickets_dir).status == "open"

    # spec: ticket-lifecycle requirement=cancel-command scenario=cancel-batch-aborts-when-one-target-has-non-terminal-children
    def test_cancel_batch_aborts_when_target_blocked(self, tmp_path: Path) -> None:
        from tiquette.store import read_ticket, write_ticket
        ...
        result = run_tq(
            "cancel", "test-0001", "par-0001", env={"TICKETS_DIR": str(tickets_dir)}
        )
        assert result.returncode != 0
        assert "par-0001" in result.stderr
        assert read_ticket("test-0001", tickets_dir).status == "open"
    ...
    # spec: ticket-lifecycle requirement=transition-output scenario=multi-id-transition-prints-every-affected-id
    def test_multi_id_prints_every_id(self, tmp_path: Path) -> None:
        from tiquette.store import write_ticket
        ...
        assert result.returncode == 0
        assert "test-0001" in result.stdout
        assert "test-0002" in result.stdout

    # spec: ticket-lifecycle requirement=close-command scenario=close-multiple-tickets
    def test_force_close_dedups_target_that_is_also_descendant(
        self, tmp_path: Path
    ) -> None:
        # par-0002 is both an explicit target and a forced descendant of par-0001;
        # it must end up closed and appear exactly once in stdout.
        from tiquette.store import read_ticket, write_ticket
        ...
        write_ticket(_make_ticket("par-0002", parent="par-0001"), tickets_dir)
        result = run_tq(
            "close", "-f", "par-0001", "par-0002", env={"TICKETS_DIR": str(tickets_dir)}
        )
        assert result.returncode == 0
        assert read_ticket("par-0001", tickets_dir).status == "closed"
        assert read_ticket("par-0002", tickets_dir).status == "closed"
        assert result.stdout.count("par-0002") == 1

    # spec: ticket-lifecycle requirement=transition-output scenario=failed-batch-transition-prints-nothing
    def test_failed_batch_prints_nothing(self, tmp_path: Path) -> None:
        from tiquette.store import write_ticket
...
class TestCascadeOrder:
    ...
    def test_parent_still_open_if_write_raises_before_parent(
        ...
        write_ticket(
            Ticket(id="chd-0001", title="Child", parent="par-0001"), tickets_dir
        )
        ...
        with patch.object(lc, "write_ticket", side_effect=failing_write):
            ...
            ns = argparse.Namespace(
                id=["par-0001"], target_status=lc.Status.CLOSED, force=True, dir=None
            )
            try:
...
class TestCreateWithNote:
    ...
    def test_create_with_note_creates_notes_section(
        self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        monkeypatch.chdir(tmp_path)
        tickets_dir = tmp_path / ".tickets"
    ...
    # spec: ticket-lifecycle requirement=create-ticket scenario=create-with-note
    def test_create_with_note_timestamp_matches_created(
        self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        import re as _re
        from datetime import datetime
    ...
    def test_create_with_multiple_notes_share_timestamp(
        self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        import re as _re
...
class TestStatusTransitionNotes:
    ...
    def test_cancel_with_note(self, tmp_path: Path) -> None:
        ...
        td = self._setup(tmp_path)
        write_ticket(_make_ticket("t-001"), td)
        r = run_tq("cancel", "t-001", "--note", "wontfix", env={"TICKETS_DIR": str(td)})
    ...
    def test_multi_id_transition_writes_notes_on_every_ticket(
        self, tmp_path: Path
    ) -> None:
        from tiquette.store import write_ticket
    ...
    def test_force_cascade_propagates_note(self, tmp_path: Path) -> None:
        ...
        write_ticket(
            _make_ticket("par-0003", parent="par-0002", status="in_progress"), td
        )
        r = run_tq(
            "close",
            "-f",
    ...
    def test_force_cascade_skips_terminal_descendants(self, tmp_path: Path) -> None:
        ...
        r = run_tq(
            "close", "-f", "par-0001", "--note", "rollup", env={"TICKETS_DIR": str(td)}
        )
        assert r.returncode == 0
...
class TestIdempotentTransitionRejection:
    ...
    def test_batch_aborts_atomically_when_one_already_at_target(
        self, tmp_path: Path
    ) -> None:
