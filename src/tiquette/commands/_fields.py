...
def _add_shared_field_flags(parser: argparse.ArgumentParser) -> None:
    parser.add_argument("-d", "--description", default=None, help="Description (markdown body)")
    parser.add_argument(
        "-t", "--type", default=None, choices=VALID_TYPES,
        help="Type (bug|feature|task|epic|chore)",
    )
    parser.add_argument(
        "-p", "--priority", default=None, choices=VALID_PRIORITIES,
    ...
    parser.add_argument("--tag", action="append", default=None, help="Tag (repeat for multiple)")
    parser.add_argument("--dep", action="append", default=None, help="Blocker ID (repeat for multiple)")
    parser.add_argument("--link", action="append", default=None, help="Associated ticket ID (symmetric, repeatable)")
    parser.add_argument("--note", action="append", default=None, help="Timestamped note (repeatable)")
...
def add_edit_flags(parser: argparse.ArgumentParser) -> None:
    ...
    _add_shared_field_flags(parser)
    parser.add_argument("--untag", action="append", default=None, help="Remove tag (repeatable)")
    parser.add_argument("--undep", action="append", default=None, help="Remove blocker (repeatable)")
    parser.add_argument("--unlink", action="append", default=None, help="Remove association (repeatable)")
    parser.add_argument(
        "--unset", action="append", default=None, choices=UNSET_TARGETS,
...
def namespace_to_field_changes(args: argparse.Namespace, *, edit_mode: bool) -> FieldChanges:
    priority_val: int | None = int(args.priority) if args.priority is not None else None
    return FieldChanges(
