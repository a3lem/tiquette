...
- fixed: `_handle_status` no longer silently swallows `TicketsNotFoundError`; the error now propagates to the top-level handler in `cli.py` which prints a diagnostic and exits 1 (tiqt-cac3)
...
- fixed: `_ticket_to_dict` return-type annotation is honest; status is coerced to `str` at the boundary (tiqt-0d9b)
